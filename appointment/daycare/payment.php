...
$stmt->execute();
$result = $stmt->get_result();
$service_cost = $result->fetch_assoc() ?? ['total_service_cost' => 0]; // Ensure default value
$stmt->close();

$total_service_cost = $service_cost['total_service_cost'] ?? 0;

// Retrieve additional hours from appointment query
$additional_hours = $appointment['additional_hours'] ?? 0;

$extra_charge = $additional_hours * 100;
// Add extra charge to appointment rate
$appointment_rate_with_extra = $appointment['apt_rate'] + $extra_charge;

// Calculate total amount
$total_amount = $appointment_rate_with_extra + $total_service_cost;
...
// Set payment statuses based on selection
...
<head>
    ...
    <script>
        function toggleReferenceField() {
            ...
            cashlessFields.style.display = (paymentType == "2") ? "block" : "none";
        }
    </script>
</head>
<body>
