...
if (isset($_POST['pet_id']) && !empty($_POST['pet_id'])) {
    ...
    // Fetch available rooms for the selected pet
    $query = "SELECT r.* FROM room r 
              JOIN pet p ON r.size_id = p.size_id AND p.type_id = r.type_id 
              WHERE p.pet_id = ? AND r.status_id = 1";
    
    $stmt = $conn->prepare($query);
    if (!$stmt) {
        die("Database error: " . $conn->error); // Debugging: Check for query preparation errors
    }
    $stmt->bind_param("i", $selected_pet_id);
    $stmt->execute();
    $rooms = $stmt->get_result();

    // Fetch available durations based on pet size
    $query = "SELECT ar.aptrate_id, d.duration, ar.rate 
          ...
          WHERE p.pet_id = ? AND d.duration_id != 4";
...
?>
    ...
    <form action="/FurCareHub/appointment/daycare/select_services.php" method="POST">
        <input type="hidden" name="pet_id" value="<?php echo $selected_pet_id; ?>">
        <input type="hidden" name="apttype_id" value="1">
        ...
        <select name="duration_id" id="duration_id" required onchange="toggleAdditionalHoursSection(); updateCheckoutDateTime();">
        ...
        <div id="additional_hours_section" style="display: none;">
            <input type="checkbox" name="add_additional_hours" id="add_additional_hours" value="1" onchange="toggleAdditionalHoursInput(); updateCheckoutDateTime();">
            <label for="add_additional_hours">Add additional hours?</label>
            <input type="number" name="additional_hours" id="additional_hours" min="1" max="5" placeholder="Enter additional hours" style="display: none;">
        </div>

        <label for="check_out_date">Check-Out Date:</label>
        <input type="date" name="check_out_date" id="check_out_date" readonly>

        <label for="check_out_time">Check-Out Time:</label>
        <input type="time" name="check_out_time" id="check_out_time" readonly>
